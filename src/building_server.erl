...
init([Servers]) ->
  ...
  State = {building_table, Parameters},
  {ok, State}.


handle_call({add, Role_id, T_id}, _From, {Table, Parameters}) ->
  ...
  {reply, Reply, {Table, Parameters}};
handle_call(get_all, _From, {Table, Parameters}) ->
  Buildings = ets:match(Table, '$1'),
  Reply = {ok, Buildings},
  {reply, Reply, {Table, Parameters}};
handle_call({get_all, Role_id}, _From, {Table, Parameters}) ->
  Buildings = ets:select(Table,[{{'_',{'$1','$2','$3','$4','$5','$6'}},[{'=:=','$3',Role_id}],['$_']}]),
  Reply = {ok, Buildings},
  {reply, Reply, {Table, Parameters}};
handle_call({get, Id}, _From, {Table, Parameters}) ->
  [{_, Building} | _] = ets:lookup(Table, Id),
  Reply = {ok, Building},
  {reply, Reply, {Table, Parameters}};
handle_call({upgrade, Id}, _From, {Table, Parameters}) ->
  ...
  if
    Building#building.level < MaxLevel ->
    ...
    true ->
      Reply = {error, reached_max_level}
  end,
  {reply, Reply, {Table, Parameters}};
handle_call({upgrade_immediately, Id}, _From, {Table, Parameters}) ->
  [{Id, Building}] = ets:lookup(Table, Id),
  ...
  {reply, Reply, {Table, Parameters}};
handle_call({get_building_num, Role_id, T_id}, _From, {Table, Parameters}) ->
  Reply = {ok, building_num(Table, Role_id, T_id)},
  {reply, Reply, {Table, Parameters}};


handle_call(test, _From, {Table, Parameters}) ->
  io:format("test~n"),
  ...
  {reply, ok, {Table, Parameters}};
handle_call({test2, Arg}, _From, {Table, Parameters}) ->
  io:format("test2:~p~n", [?MODULE]),
  {reply, Arg, {Table, Parameters}};
handle_call({test, Arg}, _From, {Table, Parameters}) ->
